...
class ApiProvider {
  ...
  ApiProvider() {
    ...
    _dio.interceptors.add(InterceptorsWrapper(onRequest: (options, handler) {
      ...
      if (e.response != null) {
        if (e.response?.data.toString().trim() == "forbidden" ||
  ...
  Future<CommonModel?> updateTransactionStatus(
      {String? status, String? utr}) async {
    String token = await appUtils.getFCMToken();
    print("Bearer $token");
    try {
      Response response = await _dio.patch("/merchant/upiTransactionStatus",
          queryParameters: {"trnx_status": status, "utr": utr},
          options: Options(
              contentType: Headers.jsonContentType,
              headers: {"Authorization": "Bearer $token"}));
      print("--------------------" + "${response.data.toString()}");
  ...
  Future<PaywallListModel?> paywallList() async {
    ...
    try {
      Response response = await _dio.get(
          "https://www.lnbits.com/paywall/api/v1/paywalls",
          queryParameters: {},
          options: Options(
              contentType: Headers.jsonContentType,
              headers: {"X-Api-Key": invoiceKey}));
      print("--------------------" + "${response.data.toString()}");
      return PaywallListModel.fromJson({"data": response.data});
    } on DioError catch (error, stacktrace) {
      if (error.response != null)
  ...
  Future<CreatePaywallModel?> createPaywall(
      {int? amount, String? desc, String? memo, String? url}) async {
    ...
    try {
      ...
      print(response.data.toString());
      return CreatePaywallModel.fromJson({"data": response.data});
    } on DioError catch (error, stacktrace) {
      if (error.response != null)
        return CreatePaywallModel.fromJson(error.response!.data);
      print("Exception occured: $error stackTrace: $stacktrace");
      return CreatePaywallModel.fromJson({"data": null});
    }
  }
  ...
  Future<CommonModel?> deletePaywall(String paywallId) async {
    ...
    try {
      Response response = await _dio.delete(
          "https://www.lnbits.com/paywall/api/v1/paywalls/$paywallId",
          queryParameters: {},
          options: Options(
              contentType: Headers.jsonContentType,
              headers: {"X-Api-Key": invoiceKey}));
